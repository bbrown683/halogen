...
impl<B: Backend> GfxSync<B> {
    pub fn new(device : Rc<RefCell<GfxDevice<B>>>, image_count : u32) -> Self {
        ...
        while current_count < image_count {
            fences.push(device.borrow().logical_device.create_fence(true).expect("Failed to create fence."));
            acquire_semaphores.push(device.borrow().logical_device.create_semaphore().expect("Failed to create semaphore."));
...
impl<B: Backend> Drop for GfxSync<B> {
    fn drop(&mut self) {
        for fence in self.fences.take().unwrap() {
            &self.device.borrow().logical_device.destroy_fence(fence);
        }
        debug_assert!(self.fences.is_none());
        for acquire_semaphore in self.acquire_semaphores.take().unwrap() {
            &self.device.borrow().logical_device.destroy_semaphore(acquire_semaphore);
        }
